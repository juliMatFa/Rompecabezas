...
var instrucciones = ["El objetivo del juego es mover las piezas hasta llegar a la imagen debajo de estas instrucciones", "Utiliza las flechas del teclado para realizar movimientos"];
...
function listarMovimiento(direccion) {
...
function chequearSiGano() {
  ...
  for (var i = 0; i < grilla.length; i++) {
    for (var j = 0; j < grilla[i].length; j++) {
      ...
      if (compararGrilla[i][j] !== grilla[i][j]) {
        return false
      }
    }
  }
